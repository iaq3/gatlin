...
class HSVMask:
	def __init__(self, color, shape_name, camera, mask, num_blobs=1, calibrated=True):
		...
		self.BASE_FRAME = "left_hand_camera"
		self.CAMERA_FRAME = "left_hand_camera"
...
class Vision:
	...
	def __init__(self):
		...
		self.green_kinect_mask = HSVMask(
			...
			"circle",
			"hand",
			{'H': {'max': 91, 'min': 66}, 'S': {'max': 82, 'min': 26}, 'D': {'max': 10000.0, 'min': -1.0}, 'V': {'max': 57, 'min': 27}},
			calibrated=True, num_blobs = 1
		)
		...
		self.pixel_radius = .4/13.53 # meters/pixel 2.1539 #radius in pixels at 1 meter of orange ball
